...
namespace Lucene.Net.Index
{
	public class TestAddIndexes : LuceneTestCase
	{
		...
		public virtual void TestSimpleCase()
		{
			...
			AddDocs2(writer, 50);
			NUnit.Framework.Assert.AreEqual(50, writer.MaxDoc());
			writer.Close();
			...
			NUnit.Framework.Assert.AreEqual(190, writer.MaxDoc());
			writer.Close();
			TestUtil.CheckIndex(dir);
			...
			NUnit.Framework.Assert.AreEqual(230, writer.MaxDoc());
			writer.Close();
			// make sure the new index is correct
			...
			NUnit.Framework.Assert.AreEqual(231, writer.MaxDoc());
			writer.Close();
		...
		public virtual void TestNoTailSegments()
		{
			...
			AddDocs(writer, 10);
			writer.AddIndexes(aux);
			NUnit.Framework.Assert.AreEqual(1040, writer.MaxDoc());
			NUnit.Framework.Assert.AreEqual(1000, writer.GetDocCount(0));
			writer.Close();
			// make sure the index is correct
		...
		public virtual void TestNoCopySegments()
		{
			...
			AddDocs(writer, 2);
			writer.AddIndexes(aux);
			NUnit.Framework.Assert.AreEqual(1032, writer.MaxDoc());
			NUnit.Framework.Assert.AreEqual(1000, writer.GetDocCount(0));
			writer.Close();
			// make sure the index is correct
		...
		public virtual void TestNoMergeAfterCopy()
		{
			...
			writer.AddIndexes(aux, new MockDirectoryWrapper(Random(), new RAMDirectory(aux, NewIOContext
				(Random()))));
			NUnit.Framework.Assert.AreEqual(1060, writer.MaxDoc());
			NUnit.Framework.Assert.AreEqual(1000, writer.GetDocCount(0));
			writer.Close();
			// make sure the index is correct
		...
		public virtual void TestMergeAfterCopy()
		{
			...
			NUnit.Framework.Assert.AreEqual(10, reader.NumDocs());
			reader.Close();
			...
			writer.AddIndexes(aux, new MockDirectoryWrapper(Random(), new RAMDirectory(aux, NewIOContext
				(Random()))));
			NUnit.Framework.Assert.AreEqual(1020, writer.MaxDoc());
			NUnit.Framework.Assert.AreEqual(1000, writer.GetDocCount(0));
			writer.Close();
			dir.Close();
			aux.Close();
		...
		public virtual void TestMoreMerges()
		{
			...
			NUnit.Framework.Assert.AreEqual(3, writer.GetSegmentCount());
			writer.Close();
			IndexWriterConfig dontMergeConfig = new IndexWriterConfig(TEST_VERSION_CURRENT, new 
				MockAnalyzer(Random())).SetMergePolicy(NoMergePolicy.COMPOUND_FILES);
			...
			writer.Close();
			IndexReader reader = DirectoryReader.Open(aux);
			NUnit.Framework.Assert.AreEqual(3, reader.NumDocs());
			reader.Close();
			...
			NUnit.Framework.Assert.AreEqual(22, reader.NumDocs());
			reader.Close();
			writer = NewWriter(dir, ((IndexWriterConfig)NewIndexWriterConfig(TEST_VERSION_CURRENT
				, new MockAnalyzer(Random())).SetOpenMode(IndexWriterConfig.OpenMode.APPEND).SetMaxBufferedDocs
		...
		private void VerifyNumDocs(Directory dir, int numDocs)
		{
			...
			NUnit.Framework.Assert.AreEqual(numDocs, reader.MaxDoc());
			NUnit.Framework.Assert.AreEqual(numDocs, reader.NumDocs());
			reader.Close();
		}
		...
		private void VerifyTermDocs(Directory dir, Term term, int numDocs)
		{
			...
			NUnit.Framework.Assert.AreEqual(numDocs, count);
			reader.Close();
		}
		...
		private void SetUpDirs(Directory dir, Directory aux, bool withID)
		{
			...
			NUnit.Framework.Assert.AreEqual(1000, writer.MaxDoc());
			NUnit.Framework.Assert.AreEqual(1, writer.GetSegmentCount());
			writer.Close();
			writer = NewWriter(aux, ((IndexWriterConfig)NewIndexWriterConfig(TEST_VERSION_CURRENT
				, new MockAnalyzer(Random())).SetOpenMode(IndexWriterConfig.OpenMode.CREATE).SetMaxBufferedDocs
		...
		private abstract class RunAddIndexesThreads
		{
			...
			internal readonly Sharpen.Thread[] threads = new Sharpen.Thread[TestAddIndexes.RunAddIndexesThreads
				.NUM_THREADS];

			/// <exception cref="System.Exception"></exception>
			public RunAddIndexesThreads(TestAddIndexes _enclosing, int numCopy)
			{
				...
				for (int i = 0; i < TestAddIndexes.RunAddIndexesThreads.NUM_INIT_DOCS; i++)
				{
					this._enclosing.AddDoc(writer);
				}
				writer.Close();
			...
			{
				for (int i = 0; i < TestAddIndexes.RunAddIndexesThreads.NUM_THREADS; i++)
				{
					this.threads[i] = new _Thread_686(this, numIter);
				}
				// System.out.println(Thread.currentThread().getName() + ": iter j=" + j);
				for (int i_1 = 0; i_1 < TestAddIndexes.RunAddIndexesThreads.NUM_THREADS; i_1++)
				{
					this.threads[i_1].Start();
				}
			}

			private sealed class _Thread_686 : Sharpen.Thread
			{
				public _Thread_686(RunAddIndexesThreads _enclosing, int numIter)
				{
					this._enclosing = _enclosing;
				...
				public override void Run()
				{
					try
					{
						Directory[] dirs = new Directory[this._enclosing.NUM_COPY];
						for (int k = 0; k < this._enclosing.NUM_COPY; k++)
						{
							dirs[k] = new MockDirectoryWrapper(LuceneTestCase.Random(), new RAMDirectory(this
								._enclosing.dir, LuceneTestCase.NewIOContext(LuceneTestCase.Random())));
						}
						int j = 0;
						while (true)
						{
				...
				private readonly RunAddIndexesThreads _enclosing;
		...
		private class CommitAndAddIndexes : TestAddIndexes.RunAddIndexesThreads
		{
			/// <exception cref="System.Exception"></exception>
			public CommitAndAddIndexes(TestAddIndexes _enclosing, int numCopy) : base(_enclosing
				)
			{
				this._enclosing = _enclosing;
			}
			...
			internal override void DoBody(int j, Directory[] dirs)
			{
				switch (j % 5)
				{
					case 0:
					{
						...
						{
							System.Console.Out.WriteLine(Sharpen.Thread.CurrentThread().GetName() + ": TEST: addIndexes(Dir[]) then full merge"
								);
						}
					...
					case 1:
					{
						if (LuceneTestCase.VERBOSE)
						{
							System.Console.Out.WriteLine(Sharpen.Thread.CurrentThread().GetName() + ": TEST: addIndexes(Dir[])"
								);
						}
						this.writer2.AddIndexes(dirs);
						break;
					...
					case 2:
					{
						if (LuceneTestCase.VERBOSE)
						{
							System.Console.Out.WriteLine(Sharpen.Thread.CurrentThread().GetName() + ": TEST: addIndexes(IndexReader[])"
								);
						}
					...
					case 3:
					{
						if (LuceneTestCase.VERBOSE)
						{
							System.Console.Out.WriteLine(Sharpen.Thread.CurrentThread().GetName() + ": TEST: addIndexes(Dir[]) then maybeMerge"
								);
						}
						this.writer2.AddIndexes(dirs);
					...
					case 4:
					{
						if (LuceneTestCase.VERBOSE)
						{
							System.Console.Out.WriteLine(Sharpen.Thread.CurrentThread().GetName() + ": TEST: commit"
								);
						}
						this.writer2.Commit();
					}
		...
		public virtual void TestAddIndexesWithThreads()
		{
			...
			int NUM_COPY = 3;
			TestAddIndexes.CommitAndAddIndexes c = new TestAddIndexes.CommitAndAddIndexes(this
				, NUM_COPY);
			...
			NUnit.Framework.Assert.AreEqual("expected num docs don't match - failures: " + c.
				failures, expectedNumDocs, c.writer2.NumDocs());
			c.Close(true);
			NUnit.Framework.Assert.IsTrue("found unexpected failures: " + c.failures, c.failures
				.IsEmpty());
			IndexReader reader = DirectoryReader.Open(c.dir2);
			NUnit.Framework.Assert.AreEqual(expectedNumDocs, reader.NumDocs());
			reader.Close();
			c.CloseDir();
		}
		...
		{
			/// <exception cref="System.Exception"></exception>
			public CommitAndAddIndexes2(TestAddIndexes _enclosing, int numCopy) : base(_enclosing
				)
			{
				this._enclosing = _enclosing;
			}
		...
		public virtual void TestAddIndexesWithClose()
		{
			...
			c.JoinThreads();
			c.CloseDir();
			NUnit.Framework.Assert.IsTrue(c.failures.Count == 0);
		}

		private class CommitAndAddIndexes3 : TestAddIndexes.RunAddIndexesThreads
		{
			/// <exception cref="System.Exception"></exception>
			public CommitAndAddIndexes3(TestAddIndexes _enclosing, int numCopy) : base(_enclosing
				)
			{
				this._enclosing = _enclosing;
			}
			...
			internal override void Handle(Exception t)
			{
				...
				{
					if (t is FileNotFoundException || t is NoSuchFileException)
					{
						report = !this.didClose;
					}
					else
		...
		{
			int NUM_COPY = 50;
			TestAddIndexes.CommitAndAddIndexes3 c = new TestAddIndexes.CommitAndAddIndexes3(this
				, NUM_COPY);
			c.LaunchThreads(-1);
			Sharpen.Thread.Sleep(TestUtil.NextInt(Random(), 10, 500));
			// Close w/o first stopping/joining the threads
			if (VERBOSE)
			{
		...
		public virtual void TestExistingDeletes()
		{
			...
			writer_1.Commit();
			NUnit.Framework.Assert.AreEqual("Documents from the incoming index should not have been deleted"
				, 1, writer_1.NumDocs());
			writer_1.Close();
		...
		public virtual void TestNonCFSLeftovers()
		{
			...
			NUnit.Framework.Assert.AreEqual("Only one compound segment should exist, but got: "
				 + Arrays.ToString(dir.ListAll()), 5, dir.ListAll().Length);
			dir.Close();
		}
		...
		public virtual void TestAddIndexMissingCodec()
		{
			...
			}
			{
				...
				{
					w.AddIndexes(toAdd);
					NUnit.Framework.Assert.Fail("no such codec");
				}
				catch (ArgumentException)
				{
				}
				...
				NUnit.Framework.Assert.AreEqual(0, open.NumDocs());
				open.Close();
				dir.Close();
			}
			try
		...
		public virtual void TestFieldNamesChanged()
		{
			...
			w.Close();
			NUnit.Framework.Assert.AreEqual(2, r3.NumDocs());
			for (int docID = 0; docID < 2; docID++)
			{
				Lucene.Net.Document.Document d = r3.Document(docID);
				if (d.Get("id").Equals("1"))
				{
					NUnit.Framework.Assert.AreEqual("doc1 field1", d.Get("f1"));
				}
				else
				{
					NUnit.Framework.Assert.AreEqual("doc2 field2", d.Get("f2"));
				}
			}
			r3.Close();
			d3.Close();
		...
		public virtual void TestAddEmpty()
		{
			...
			{
				NUnit.Framework.Assert.IsTrue("empty segments should be dropped by addIndexes", (
					(AtomicReader)ctx.Reader()).MaxDoc() > 0);
			}
			dr.Close();
			d1.Close();
		}
		...
		public virtual void TestLocksBlock()
		{
			...
			w1.AddDocument(new Lucene.Net.Document.Document());
			w1.Commit();
			Directory dest = NewDirectory();
