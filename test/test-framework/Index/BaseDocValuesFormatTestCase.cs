...
namespace Lucene.Net.TestFramework.Index
{
    ...
    {
        protected internal override void AddRandomFields(Document
             doc)
        {
            if (Usually())
            {
        ...
        private void DoTestBinaryVsStoredFields(int minLength, int maxLength)
		{
			...
			for (int i = 0; i < numDocs; i++)
			{
				idField.StringValue = i.ToString();
				int length;
				length = minLength == maxLength ? minLength : Random().NextInt(minLength, maxLength);
				var buffer = new sbyte[length];
				Random().NextBytes(buffer.ToBytes());
        ...
        private void DoTestSortedVsStoredFields(int minLength, int maxLength)
		{
			...
			for (int i = 0; i < numDocs; i++)
			{
				idField.StringValue = i.ToString();
				int length;
				length = minLength == maxLength ? minLength : TestUtil.NextInt(Random(), minLength, maxLength);
				var buffer = new sbyte[length];
				Random().NextBytes(buffer.ToBytes());
				storedField.SetBytesValue(buffer);
				dvField.SetBytesValue(buffer);
            ...
            )
        {
            ...
            for (int i = 0; i < numDocs; i++)
            {
                Document doc = new Document
                    ();
                Field idField = new StringField("id", i.ToString(), Field.Store
                    .NO);
                doc.Add(idField);
                int length;
                if (minLength == maxLength)
                ...
                var unordered = new List<string>(values);
        ...
        private void DoTestSortedSetVsUninvertedField(int minLength, int maxLength)
        {
            ...
            {
                var doc = new Document();
                Field idField = new StringField("id", i.ToString(), Field.Store.NO);
                doc.Add(idField);
                int length;
                if (minLength == maxLength)
                {
                ...
                {
                    // fixed length
                    length = Random().NextInt(minLength, maxLength);
                }
                ...
                // add in any order to the dv field
                var unordered2 = new List<string>(values);
        ...
        public virtual void TestGCDCompression()
        {
            ...
            for (int i = 0; i < numIterations; i++)
            {
                ...
                BaseDocValuesFormatTestCase.LongProducer longs = new AnonymousLongProducer3(min, mul);
                DoTestNumericsVsStoredFields(longs);
            }
        }

        private sealed class AnonymousLongProducer3 : LongProducer
        {
            public AnonymousLongProducer3(long min, long mul)
            {
                this.min = min;
                this.mul = mul;
            }
        ...
        public virtual void TestZeroOrMin()
        {
            ...
            for (int i = 0; i < numIterations; i++)
            {
                BaseDocValuesFormatTestCase.LongProducer longs = new AnonymousLongProducer4();
                DoTestNumericsVsStoredFields(longs);
            }
        }

        private sealed class AnonymousLongProducer4 : LongProducer
        {
            public AnonymousLongProducer4()
            {
            }
        ...
        public virtual void TestTwoNumbersOneMissing()
        {
            ...
            var doc = new Document { new StringField("id", "0", Field.Store.YES), new NumericDocValuesField("dv1", 0) };
            iw.AddDocument(doc);
            doc = new Document { new StringField("id", "1", Field.Store.YES) };
            iw.AddDocument(doc);
            iw.ForceMerge(1);
            iw.Close();
        ...
        public virtual void TestTwoNumbersOneMissingWithMerging()
        {
            ...
            IndexWriterConfig conf = NewIndexWriterConfig(TEST_VERSION_CURRENT, null);
            conf.SetMergePolicy(NewLogMergePolicy());
            var iw = new RandomIndexWriter(Random(), directory, conf);
            var doc = new Document { new StringField("id", "0", Field.Store.YES), new NumericDocValuesField("dv1", 0) };
            iw.AddDocument(doc);
            iw.Commit();
            doc = new Document { new StringField("id", "1", Field.Store.YES) };
        ...
        public virtual void TestTwoBytesOneMissing()
        {
            ...
            conf.SetMergePolicy(NewLogMergePolicy());
            var iw = new RandomIndexWriter(Random(), directory, conf);
            var doc = new Document { new StringField("id", "0", Field.Store.YES), new BinaryDocValuesField("dv1", new BytesRef()) };
            iw.AddDocument(doc);
            doc = new Document { new StringField("id", "1", Field.Store.YES) };
            iw.AddDocument(doc);
            iw.ForceMerge(1);
        ...
        public virtual void TestThreeBytesOneMissingWithMerging()
        {
            ...
            Document doc = new Document
			{
			    new StringField("id", "0", Field.Store.YES),
			    new BinaryDocValuesField("dv1", new BytesRef())
			};
            iw.AddDocument(doc);
            doc = new Document();
            doc.Add(new StringField("id", "1", Field.Store.YES));
        ...
        public virtual void TestHugeBinaryValues()
        {
            ...
            if (doFixed)
            {
                ...
                numDocs = Random().NextInt(10, 20);
                fixedLength = Random().NextInt(65537, 256 * 1024);
            }
            else
            {
                numDocs = Random().NextInt(100, 200);
            }
            ...
            for (int docID = 0; docID < numDocs; docID++)
            {
                ...
                docBytes.Add(bytes);
                Document doc = new Document();
                BytesRef b = new BytesRef(bytes.ToSbytes()) { length = bytes.Length };
        ...
        public virtual void TestHugeBinaryValueLimit()
        {
            ...
            for (int docID = 0; docID < numDocs; docID++)
            {
                ...
                else
                {
                    ...
                    {
                        numBytes = Random().NextInt(1, Lucene42DocValuesFormat.MAX_BINARY_FIELD_LENGTH);
                    }
                }
                totalBytes += numBytes;
                if (totalBytes > 5 * 1024 * 1024)
        ...
        public virtual void TestThreads2()
		{
			...
			for (int i = 0; i < numDocs; i++)
			{
				...
				storedNumericField.StringValue = System.Convert.ToString(numericValue);
				dvNumericField.SetLongValue(numericValue);
				Document doc = new Document {idField};
			    if (Random().Next(4) > 0)
				{
					doc.Add(storedBinField);
					doc.Add(dvBinField);
                    ...
                    foreach (AtomicReaderContext context in ir.Leaves)
                    {
                        ...
                        for (int j = 0; j < r.MaxDoc; j++)
                        {
                            ...
                            else
                            {
                                if (binaries != null)
                                {
                                    IsFalse(binaryBits[j]);
                                    IsFalse(sortedBits[j]);
