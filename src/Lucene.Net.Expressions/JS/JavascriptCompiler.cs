...
namespace Lucene.Net.Expressions.JS
{
	...
	public class JavascriptCompiler
	{
		...
		private static readonly string COMPILED_EXPRESSION_CLASS = typeof(JavascriptCompiler
			).FullName + "$CompiledExpression";

		private static readonly string COMPILED_EXPRESSION_INTERNAL = COMPILED_EXPRESSION_CLASS
			.Replace('.', '/');

		private static readonly Type EXPRESSION_TYPE = Type.GetType(typeof(Expression));

		private static readonly Type FUNCTION_VALUES_TYPE = Type.GetType(typeof(FunctionValues
			));

		private static readonly Method EXPRESSION_CTOR = GetMethod("void <init>(String, String[])"
			);

		private static readonly Method EVALUATE_METHOD = GetMethod("double evaluate(int, "
			 + typeof(FunctionValues).FullName + "[])");

		private static readonly Method DOUBLE_VAL_METHOD = GetMethod("double doubleVal(int)"
			);
		...
		private readonly ClassWriter classWriter = new ClassWriter(ClassWriter.COMPUTE_FRAMES
		...
		private Expression CompileExpression(ClassLoader parent)
		{
			try
			{
				Org.Antlr.Runtime.Tree.Tree antlrTree = GetAntlrComputedExpressionTree();
				BeginCompile();
				RecursiveCompile(antlrTree, Type.DOUBLE_TYPE);
				EndCompile();
				Type evaluatorClass = new JavascriptCompiler.Loader(parent).Define(COMPILED_EXPRESSION_CLASS
		...
		private void RecursiveCompile(Org.Antlr.Runtime.Tree.Tree current, Type expected)
		{
			...
			switch (type)
			{
				...
				{
					Org.Antlr.Runtime.Tree.Tree identifier = current.GetChild(0);
					string call = identifier.GetText();
					int arguments = current.GetChildCount() - 1;
					MethodInfo method = functions.Get(call);
					if (method == null)
					{
						throw new ArgumentException("Unrecognized method call (" + call + ").");
					}
					int arity = Sharpen.Runtime.GetParameterTypes(method).Length;
					if (arguments != arity)
					{
		...
		private void PushArith(int @operator, Org.Antlr.Runtime.Tree.Tree current, Type expected
			)
		{
			PushBinaryOp(@operator, current, expected, Type.DOUBLE_TYPE, Type.DOUBLE_TYPE, Type
				.DOUBLE_TYPE);
		}

		private void PushShift(int @operator, Org.Antlr.Runtime.Tree.Tree current, Type expected
			)
		{
		...
		private void PushBitwise(int @operator, Org.Antlr.Runtime.Tree.Tree current, Type
			 expected)
		{
		...
		private void PushBinaryOp(int @operator, Org.Antlr.Runtime.Tree.Tree current, Type
			 expected, Type arg1, Type arg2, Type returnType)
		{
		...
		private void PushCond(int @operator, Org.Antlr.Runtime.Tree.Tree current, Type expected
			)
		{
		...
		private Org.Antlr.Runtime.Tree.Tree GetAntlrComputedExpressionTree()
		{
