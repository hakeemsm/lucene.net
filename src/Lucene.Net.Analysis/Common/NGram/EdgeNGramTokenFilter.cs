...
namespace Lucene.Net.Analysis.NGram
{
    ...
    public sealed class EdgeNGramTokenFilter : TokenFilter
    {
            ...
            : base(input)
        {
            ...
            this.charUtils = version.OnOrAfter(Version.LUCENE_44) ? CharacterUtils
                .GetInstance(version) : CharacterUtils.DOTNET;
        ...
        public EdgeNGramTokenFilter(Version version,TokenStream input, string sideLabel, int minGram, int maxGram)
            : this(version,input, SideExtensions.GetSide(sideLabel), minGram, maxGram)
        {
        }

		/// <summary>Creates EdgeNGramTokenFilter that can generate n-grams in the sizes of the given range
		/// 	</summary>
		...
		/// <param name="minGram">the smallest n-gram to generate</param>
		/// <param name="maxGram">the largest n-gram to generate</param>
		public EdgeNGramTokenFilter(Version version, TokenStream input, int minGram, int 
			maxGram) : this(version, input, Side.FRONT, minGram, maxGram
			)
		{
		}
            ...
            while (true)
            {
                ...
                if (curGramSize <= maxGram)
                {
					if (curGramSize <= curCodePointCount)
					{
						...
						int start = side == Side.FRONT ? 0 : charUtils.OffsetByCodePoints
							(curTermBuffer, 0, curTermLength, curTermLength, -curGramSize);
