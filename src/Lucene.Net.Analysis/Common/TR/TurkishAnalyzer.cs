...
namespace Lucene.Net.Analysis.TR
{
	...
	public sealed class TurkishAnalyzer : StopwordAnalyzerBase
	{
		...
		/// <summary>File containing default Turkish stopwords.</summary>
		/// <remarks>File containing default Turkish stopwords.</remarks>
		...
		{
			return TurkishAnalyzer.DefaultSetHolder.DEFAULT_STOP_SET;
		}
		...
		private class DefaultSetHolder
		{
			...
			static DefaultSetHolder()
			{
				try
				{
					DEFAULT_STOP_SET = LoadStopwordSet(false, typeof(TurkishAnalyzer), DEFAULT_STOPWORD_FILE
						, STOPWORDS_COMMENT);
				}
				catch (IOException)
				{
		...
		/// .
		/// </summary>
		protected internal TurkishAnalyzer(Version matchVersion) : this(matchVersion, TurkishAnalyzer.DefaultSetHolder
			.DEFAULT_STOP_SET)
		{
		}
		...
		protected internal TurkishAnalyzer(Version matchVersion, CharArraySet stopwords) : 
		...
		public TurkishAnalyzer(Version matchVersion, CharArraySet stopwords, CharArraySet
			...
			, TextReader reader)
		{
			...
			if (matchVersion.Value.OnOrAfter( Version.LUCENE_48))
			{
