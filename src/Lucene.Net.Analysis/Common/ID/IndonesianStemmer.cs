...
{
	/// <summary>Stemmer for Indonesian.</summary>
	/// <remarks>
	...
	public class IndonesianStemmer
	{
		...
		/// <summary>Stem a term (returning its new length).</summary>
		/// <remarks>
		...
		private int RemoveSecondOrderPrefix(char[] text, int length)
		{
			...
			if (StemmerUtil.StartsWith(text, length, "be") && length > 4 && !IsVowel(text[2])
				 && text[3] == 'e' && text[4] == 'r')
			{
				flags |= REMOVED_BER;
				numSyllables--;
		...
		{
			if (StemmerUtil.EndsWith(text, length, "kan") && (flags & REMOVED_KE) == 0 && (flags
				 & REMOVED_PENG) == 0 && (flags & REMOVED_PE) == 0)
			{
				numSyllables--;
				return length - 3;
			}
			if (StemmerUtil.EndsWith(text, length, "an") && (flags & REMOVED_DI) == 0 && (flags
				 & REMOVED_MENG) == 0 && (flags & REMOVED_TER) == 0)
			{
				numSyllables--;
				return length - 2;
			}
			if (StemmerUtil.EndsWith(text, length, "i") && !StemmerUtil.EndsWith(text, length
				, "si") && (flags & REMOVED_BER) == 0 && (flags & REMOVED_KE) == 0 && (flags & REMOVED_PENG
				) == 0)
			{
				numSyllables--;
				return length - 1;
