...
namespace Lucene.Net.Util
{
	...
	public sealed class PForDeltaDocIdSet : DocIdSet
	{
		...
		internal static readonly Lucene.Net.Util.PForDeltaDocIdSet EMPTY = new Lucene.Net.Util.PForDeltaDocIdSet
		...
		public class Builder
		{
			...
			internal virtual void ComputeFreqs()
			{
				...
				{
					++freqs[32 - Sharpen.Extensions.NumberOfLeadingZeros(buffer[i])];
				}
			}
				...
				)
			{
				...
				long blockSize = 1 + PackedInts.FormatHelper.ByteCount(format, PackedInts.VERSION_CURRENT
					, BLOCK_SIZE, bitsPerValue);
				// header: number of bits per value
				if (numExceptions > 0)
				{
					blockSize += 2 + numExceptions + PackedInts.FormatHelper.ByteCount(format, PackedInts
			...
			internal virtual int ComputeOptimalNumberOfBits()
			{
				...
				for (int bitsPerValue = this.bitsPerValue - 1; bitsPerValue >= 0 && numExceptions
					 <= MAX_EXCEPTIONS; numExceptions += freqs[bitsPerValue--])
				{
					int newBlockSize = PforBlockSize(bitsPerValue, numExceptions, actualBitsPerValue 
						- bitsPerValue);
			...
			internal virtual void PforEncode()
			{
				...
				if (bitsPerValue > 0)
				{
					PackedInts.Encoder encoder = PackedInts.GetEncoder(PackedInts.Format.PACKED, PackedInts
				...
				if (numExceptions > 0)
				{
					...
					data.length += PackedInts.FormatHelper.ByteCount(PackedInts.Format.PACKED, PackedInts
						.VERSION_CURRENT, numExceptions, bitsPerException);
			...
			public virtual PForDeltaDocIdSet Build()
			{
				...
				else
				{
					...
					offsets = new MonotonicAppendingLongBuffer(initialPageCount, pageSize, PackedInts
						.COMPACT);
					// Now build the index
					PForDeltaDocIdSet.Iterator it = new PForDeltaDocIdSet.Iterator(dataArr, cardinality
...
index_break: ;
		...
		public override DocIdSetIterator Iterator()
		{
			...
			{
				return new PForDeltaDocIdSet.Iterator(data, cardinality, indexInterval, docIDs, offsets
					);
			}
		}
		...
		internal class Iterator : DocIdSetIterator
		{
			...
			{
				int bitsPerValue = token & unchecked((int)(0x1F));
				if (bitsPerValue == 0)
				{
				...
				if ((token & HAS_EXCEPTIONS) != 0)
				{
					...
					offset += PackedInts.FormatHelper.ByteCount(PackedInts.Format.PACKED, PackedInts.
						VERSION_CURRENT, numExceptions, bitsPerException);
					for (int i = 0; i < numExceptions; ++i)
					{
			...
			internal virtual void UnaryDecompress(byte token)
			{
				...
				{
					byte b = data[offset++];
					for (int bitList = BitUtil.BitList(b); bitList != 0; ++i, bitList = (int)(((uint)
						bitList) >> 4))
					{
						nextDocs[i] = docID + (bitList & unchecked((int)(0x0F)));
					}
					docID += 8;
				}
			}
