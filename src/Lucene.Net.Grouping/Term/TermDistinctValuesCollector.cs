...
namespace Lucene.Net.Grouping.Term
{
	...
	/// <see cref="AbstractDistinctValuesCollector{GC}"
	...
	public class TermDistinctValuesCollector : AbstractDistinctValuesCollector<TermDistinctValuesCollector.GroupCount>
	{
        ...
        public override AtomicReaderContext NextReader
		{
		    set
		    {
		        groupFieldTermIndex = FieldCache.DEFAULT.GetTermsIndex(((AtomicReader) value.Reader), groupField);
		        countFieldTermIndex = FieldCache.DEFAULT.GetTermsIndex(((AtomicReader) value.Reader), countField);
		        ordSet.Clear();
		        foreach (GroupCount group in groups)
		        {
		            int groupOrd = group.groupValue == null
		                ? -1
		                : groupFieldTermIndex.LookupTerm(group
		                    .groupValue);
		            ...
		            int i = 0;
		            foreach (BytesRef b in group.uniqueValues)
		            {
		                int countOrd = b == null ? -1 : countFieldTermIndex.LookupTerm(b);
		                if (b == null || countOrd >= 0)
		                {
		...
		/// <lucene.experimental></lucene.experimental>
		public class GroupCount : GroupCount<BytesRef>
		{
			internal int[] ords;
