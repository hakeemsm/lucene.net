...
namespace Lucene.Net.Grouping
{
	...
	public abstract class AbstractGroupFacetCollector : Collector
	{
			...
			, BytesRef facetPrefix)
		{
			...
			segmentResults = new List<SegmentResult>();
		}
		...
		public virtual GroupedFacetResult MergeSegmentResults(int size, int minCount, bool orderByCount)
		{
			...
			int totalCount = 0;
			int missingCount = 0;
			var segments = new SegmentResultPriorityQueue(segmentResults.Count);
			...
			var facetResult = new GroupedFacetResult(size, minCount, orderByCount, totalCount, missingCount);
			while (segments.Size() > 0)
			{
				SegmentResult segmentResult_1 = segments.Top();
		...
		protected internal abstract SegmentResult CreateSegmentResult();
		...
		{
			private sealed class FacetEntryComparer : IComparer<FacetEntry>
			{
			...
			private static readonly IComparer<FacetEntry> orderByCountAndValue
				 = new FacetEntryComparer();

			private sealed class FacetEntryComparer2 : IComparer<FacetEntry>
			{
			    public int Compare(FacetEntry a, FacetEntry b)
				{
					return a.GetValue().CompareTo(b.GetValue());
			...
			private static readonly IComparer<FacetEntry> orderByValue
				 = new FacetEntryComparer2();

			private readonly int maxSize;

			private readonly NavigableSet<FacetEntry> facetEntries;
			...
			{
				this.facetEntries = new HashSet<FacetEntry>(orderByCount
			...
			public virtual void AddFacetCount(BytesRef facetValue, int count)
			{
				...
				var facetEntry = new FacetEntry(facetValue, count);
