...
namespace Lucene.Net.Grouping
{
	...
	{
		protected internal readonly IDictionary<GROUP_VALUE_TYPE, SearchGroupDocs
			<GROUP_VALUE_TYPE>> groupMap;

		private readonly int maxDocsPerGroup;

		protected internal SearchGroupDocs<GROUP_VALUE_TYPE>[] groupDocs;
			...
			, bool getMaxScores, bool fillSortFields)
		{
			...
			groupMap = new Dictionary<GROUP_VALUE_TYPE, SearchGroupDocs
				<GROUP_VALUE_TYPE>>(groups.Count);
			foreach (SearchGroup<GROUP_VALUE_TYPE> group in groups)
			{
				//System.out.println("  prep group=" + (group.groupValue == null ? "null" : group.groupValue.utf8ToString()));
				TopDocsCollector<FieldValueHitQueue.Entry> collector;
				...
				groupMap[group.groupValue] = new SearchGroupDocs<GROUP_VALUE_TYPE>(this, group.groupValue, collector));
			}
		}
		...
		protected internal abstract SearchGroupDocs<GROUP_VALUE_TYPE
		...
		public virtual TopGroups<GROUP_VALUE_TYPE> GetTopGroups(int withinGroupOffset)
		{
			...
			{
				SearchGroupDocs<GROUP_VALUE_TYPE> groupDocs = groupMap[group.groupValue];
				...
				groupDocsResult[groupIDX++] = new GroupDocs<GROUP_VALUE_TYPE>(float.NaN, topDocs.
					MaxScore, topDocs.TotalHits, topDocs.ScoreDocs, groupDocs.groupValue, group
					.sortValues);
		...
		public class SearchGroupDocs<GROUP_VALUE_TYPE>
		{
			...
			public SearchGroupDocs(AbstractSecondPassGroupingCollector<GROUP_VALUE_TYPE> _enclosing
				, GROUP_VALUE_TYPE groupValue, TopDocsCollector<FieldValueHitQueue.Entry> collector)
			{
