...
namespace Lucene.Net.Grouping
{
	...
	public class TopGroups<GROUP_VALUE_TYPE>
	{
		...
		public static TopGroups<T> Merge<T>(TopGroups
			 ...
			 scoreMergeMode)
		{
			...
			foreach (TopGroups<T> shard in shardGroups)
			{
			...
			for (int groupIDX = 0; groupIDX < numGroups; groupIDX++)
			{
				...
				for (int shardIDX = 0; shardIDX < shardGroups.Length; shardIDX++)
				{
					...
					TopGroups<T> shard_1 = shardGroups[shardIDX];
			...
			if (totalGroupCount != null)
			{
				TopGroups<T> result = new TopGroups
			...
			{
				return new TopGroups<T>(groupSort.GetSort(), docSort
