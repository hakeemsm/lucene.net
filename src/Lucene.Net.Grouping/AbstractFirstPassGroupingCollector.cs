...
namespace Lucene.Net.Grouping
{
    ...
    public abstract class AbstractFirstPassGroupingCollector<GROUP_VALUE_TYPE> : Collector where GROUP_VALUE_TYPE:class
    {
        private readonly Sort groupSort;

        private readonly FieldComparator<GROUP_VALUE_TYPE>[] comparators;

        private readonly int[] reversed;

        private readonly int topNGroups;

        private readonly Dictionary<GROUP_VALUE_TYPE, CollectedSearchGroup<GROUP_VALUE_TYPE>> groupMap;
        ...
        public AbstractFirstPassGroupingCollector(Sort groupSort, int topNGroups)
        {
            ...
            comparators = new FieldComparator<GROUP_VALUE_TYPE>[sortFields.Length];
            ...
            for (int i = 0; i < sortFields.Length; i++)
            {
                ...
                comparators[i] = sortField.GetComparator(topNGroups + 1, i) as FieldComparator<GROUP_VALUE_TYPE>;
            ...
            , bool fillFields)
        {
            ...
            ICollection<SearchGroup<GROUP_VALUE_TYPE>> result = new List<SearchGroup<GROUP_VALUE_TYPE
                >>();
            int upto = 0;
        ...
        public override void Collect(int doc)
        {
            ...
            CollectedSearchGroup<GROUP_VALUE_TYPE> group = groupMap[groupValue];
            if (group == null)
            {
                ...
                if (groupMap.Count < topNGroups)
                {
                    ...
                    var sg = new CollectedSearchGroup<GROUP_VALUE_TYPE>
                    {
                        groupValue = CopyDocGroupValue(groupValue, null),
                        comparatorSlot = groupMap.Count,
                        topDoc = docBase + doc
                    };
                    foreach (var fc in comparators)
                    {
                ...
                foreach (var fc1 in comparators)
                {
                    fc1.Copy(bottomGroup.comparatorSlot, doc);
                }
                ...
                foreach (var fc2 in comparators)
                {
        ...
        {
            IComparer<CollectedSearchGroup<object>> comparator = new CollectedSearchGroupComparerImpl(this);
            orderedGroups = new SortedSet<CollectedSearchGroup<GROUP_VALUE_TYPE>>(comparator);
        ...
        private sealed class CollectedSearchGroupComparerImpl : IComparer<CollectedSearchGroup<object>>
        {
            public CollectedSearchGroupComparerImpl(AbstractFirstPassGroupingCollector<GROUP_VALUE_TYPE> _enclosing
                )
            {
